...
import { useCanRedo, useCanUndo, useHistory, useSelf, useStorage } from '@liveblocks/react';
import { useMutation, useOthersMapped } from '@liveblocks/react';
...
export const Canvas = ({ boardId }: CanvasProps) => {
  ...
  const onLayerPointerDown = useMutation(
    ({ self, setMyPresence }, e: React.PointerEvent, layerId) => {
      ...
      if (!self.presence.selection || !self.presence.selection.includes(layerId)) {
        setMyPresence({ selection: [layerId] }, { addToHistory: true });
      }
      setCanvasState({ mode: CanvasMode.Translating, current: point });
    },
    ...
    for (const user of selections) {
      ...
      if (selection) {
        for (const layerId of selection) {
          layerIdsToColorSelection[layerId] = '#082567';
        }
      }
    }
